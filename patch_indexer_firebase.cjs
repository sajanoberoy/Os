...
const newIndexer = `import fs from 'fs';
...
export class RAGIndexer {
  ...
  public async reindexAll(): Promise<{ chunksCount: number; documentsCount: number }> {
    ...
    // 1. Process official documents from Firestore
    try {
...
fs.writeFileSync('server/rag/indexer.ts', newIndexer);
console.log('Indexer restored to Firebase');
