...
// In-memory / persistent session store for prototype
...
const getUsersPath = () => {
  ...
  if (!fs_node.existsSync(dirPath)) fs_node.mkdirSync(dirPath, { recursive: true });
  return path.join(dirPath, 'users.json');
};

const loadUsers = () => {
  const usersPath = getUsersPath();
  if (!fs_node.existsSync(usersPath)) {
    fs_node.writeFileSync(usersPath, JSON.stringify({}));
    return {};
  }
  return JSON.parse(fs_node.readFileSync(usersPath, 'utf8'));
};

const saveUsers = (users: any) => {
  fs_node.writeFileSync(getUsersPath(), JSON.stringify(users, null, 2));
};
...
export async function authenticateUser(email: string, password: string): Promise<{ success: boolean; session?: UserSession; error?: string }> {
  ...
  for (const [id, u] of Object.entries(users)) {
    if ((u as any).email === normalizedEmail) {
      user = u;
      userId = id;
      break;
    }
  ...
  // Auto-seed admin user if it doesn't exist (for initial setup)
  ...
  // If user is a student and not verified, reject and require verification
...
export async function logUserActivity(userId: string, ipAddress: string, userAgent: string, askedQuestion: boolean = false) {
  try {
    ...
    if (users[userId]) {
      ...
      users[userId].ipAddress = ipAddress;
      users[userId].userAgent = userAgent;
      if (askedQuestion) {
        users[userId].questionsCount = (users[userId].questionsCount || 0) + 1;
      }
      saveUsers(users);
    }
  } catch (err) {
...
export async function verifyUserCode(email: string, code: string): Promise<{ success: boolean; session?: UserSession; error?: string }> {
  try {
    ...
    users[userId].verificationCode = null;
    users[userId].verificationCodeExpires = null;
    users[userId].lastActive = new Date().toISOString();
    saveUsers(users);
